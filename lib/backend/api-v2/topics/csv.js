...
const apiV1 = require('lib/backend/db-api')
...
app.get('/votes-csv/:id',
middlewares.users.restrict,
middlewares.topics.findById,
middlewares.forums.findFromTopic,
middlewares.forums.privileges.canChangeTopics,
(req, res, next) => apiV1.topic.getVotes(req.topic.id).then(votes => {
  req.votes = votes
  next()
}),
function getCsv (req, res, next) {
  const { forum, topic, votes } = req

  const rows = []

  rows.push('Fecha,Nombre,Apellido,Email'.split(','))

  votes.forEach(v => rows.push([
    escapeTxt(moment(v.createdAt, '', req.locale).format('LL LT')),
    escapeTxt(v.votante.firstName),
    escapeTxt(v.votante.lastName),
    escapeTxt(v.votante.email)
  ]))
  //log(votes, rows)

  //const json2xls = require('json2xls')
  //app.use(json2xls.middleware)
  //res.xls('archivo.xlsx', rows);
  json2csv(rows, function (err, csv) {
    ...
    res.set({
      'Content-Type': 'text/csv; charset=utf-8',
      'Content-Disposition': `attachment; filename=votos-eje-${topic.id}.csv`
    })
    res.write(csv)
    res.end()
  }, { prependHeader: false })
