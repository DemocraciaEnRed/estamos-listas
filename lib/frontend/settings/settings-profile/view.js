...
export default class ProfileForm extends FormView {
  ...
  switchOn () {
    ...
    // Circulo
    this.circulosTag = this.find('select#circulo')[0]
    ...
    circuloStore.findAll({})
      .then((circulos) => {
        circulos.forEach((cir) => {
          var option = document.createElement('option')
          option.value = cir.id
          option.innerHTML = cir.name
          this.circulosTag.appendChild(option)
        })
        this.circulosTag.value = user.extra.circulo || ''
        var selected = this.find(`option[value="${this.circulosTag.value}"]`)
        selected.attr('selected', true)
      })
      .catch((err) => {
